...
void RttTcpServer::start()
{
    _threadActive = true;
    while (_threadActive) {
        LOG(INFO) << "Running in Server Mode, listen port " << _port << std::endl;
        work();
        usleep(2*1000000);
    }
}
...
bool RttTcpServer::work() {
    ...
    if (bind(sockFd, (struct sockaddr *) &serverAddr, sizeof(serverAddr)) < 0) {
        LOG(ERROR) << "ERROR on binding" << std::endl;
        return false;
    }
    ...
    while(_threadActive) {
        LOG(INFO) << "Waiting new connection.." << std::endl;
        ...
        if (connHandlerThread.joinable()) {
            LOG(DEBUG) << "Detaching thread " << connHandlerThread.get_id() << std::endl;
...
bool RttTcpServer::handleClientConnection(int clientSockFd, struct sockaddr_in clientAddr) {
    LOG(INFO) << "Got connection from " << inet_ntoa(clientAddr.sin_addr) << ":" << clientAddr.sin_port << std::endl;
    bool isClientConnected = true;

    while(isClientConnected) {
        if (!readRequestSendResponse(clientSockFd)) {
            LOG(ERROR) << "Error on client read/send!" << std::endl;
            break;
        }
    }

    LOG(INFO) << "close connection from " << inet_ntoa(clientAddr.sin_addr) << std::endl;
    close(clientSockFd);
    return true;
}
...
bool RttTcpServer::readRequestSendResponse(int clientSocketFd) {
    ...
    if (readBytes < 0) {
        LOG(ERROR) << "Error on read!" << std::endl;
        return false;
    }
    ...
    if (sentBytes < 0) {
        LOG(ERROR) << "Error on send!" << std::endl;
        return false;
    }
